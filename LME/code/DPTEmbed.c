...
int num_clusters;
bool update_cluster_assignments;

int main(int argc, char* argv[])
{
    PARAS myparas = parse_paras(argc, argv, trainfile, embeddingfile);
    PDATA pd = read_playlists_data(trainfile);
...
double** logistic_embed_allinone(PDATA pd, PARAS myparas, char* embedding_file, int* l)
{
	...
	*l = (k + m) * myparas.num_points;
    ...
    //ADITH
    if (myparas.regularization_type == 6 || myparas.regularization_type == 7){
        for(i=0; i<d; i++)
            X[0][i] = 0.0;
    }
    if (myparas.regularization_type == 7){
        cluster_ids = calloc(k, sizeof(int));
    }
        ...
        for(i = 0; i < pd.num_playlists; i ++)
        {
            if(pd.playlists_length[i] > 1)
            {
                for(j = 0; j < pd.playlists_length[i] - 1; j++)
                {
                    ...
                    temp_pair.to = pd.playlists[i][j + 1];
    ...
    for(fr = 0; fr < k; fr++)
    {
		...
		wtable_count += collection_size;
            ...
            vec_scalar_sum(p, 1.0, collection_size); //ADITH: Add 1 for smoothing

            Veccopy(p, tempk, collection_size);
            inverse_on_vec(tempk, collection_size);

            log_on_vec(p, collection_size);
            ...
            scale_vec(p, collection_size, -1.0);
            // exp(p[i]) = exp(-||delta(fr,i)||^2) / Z(fr)

            if (myparas.num_points < 3)
                for(j = 0; j < collection_size; j++)
                    for(i = 0; i < d; i++)
                        dv[j][i] = -2.0 * exp(p[j]) * delta[j][i] * tempk[j];
            else
                for(j = 0; j < collection_size; j++)
                    for(i = 0; i < d; i++) {
                        dv[j][i] = -2.0 * exp(p[j]) * delta[j][i] * tempk[j];
                        mid_dv[j][i] = -2.0 * exp(p[j]) * mid_delta[j][i] * tempk[j];
                    }
            ...
            for(to = 0; to < k; to++)
            {
                if(myparas.allow_self_transition || (!myparas.allow_self_transition && fr != to))
                {
                    ...
                    //ADITH
                    if (myparas.regularization_type == 8) idx = 1;
                    ...
                    if(idx >= 0)
                    {
                        ...
                        //ADITH
                        if (myparas.regularization_type == 8) temp_val += myparas.lambda;
    ...
    if(myparas.regularization_type == 7){
        free(cluster_ids);
        if (cluster_sizes != NULL) free(cluster_sizes);
    }
...
void update_cluster_ids(double** X, int n, int d, double lambda) {
//updates the cluster_ids array, num_centers and cluster_sizes correctly
    ...
    ClusterMeanList* centroids = malloc(sizeof(ClusterMeanList));
    memset(centroids->val, 0, d * sizeof(double));
    for (i=0; i<n; i++){
        add_vec(centroids->val, X[i], d, 1.0/n);
        cluster_ids[i] = 0;
    }
    centroids->id = 0;
    centroids->next = NULL;
    ...
    while ((potential < prev_potential) || (prev_potential < 0.0)){
        prev_potential = potential;
        potential = 0.0;
        for(i=0; i<n; i++){
            double minSeen = DBL_MAX;
            int minSeenId = -1;
            ClusterMeanList* cent_ptr = centroids;
            while (cent_ptr != NULL){
                double currDist = vec_sq_dist(X[i], cent_ptr->val, d);
                if (currDist < minSeen){
                    minSeen = currDist;
                    minSeenId = cent_ptr->id;
                }
                cent_ptr = cent_ptr->next;
            }
            if (minSeen > lambda){
                ClusterMeanList* new_node = malloc(sizeof(ClusterMeanList));
                new_node->id = centroids->id+1;
                memset(new_node->val, 0, d * sizeof(double));
                add_vec(new_node->val, X[i], d, 1.0);
                cluster_ids[i] = new_node->id;
                new_node->next = centroids;
                centroids = new_node;
            } else {
                cluster_ids[i] = minSeenId;
                potential += minSeen;
            }
        }

        int num_cls = centroids->id + 1;
        potential += lambda*num_cls;
       
        if (cluster_sizes != NULL) free(cluster_sizes);
        cluster_sizes = (int*) calloc(num_cls, sizeof(int));
        ClusterMeanList* cent_ptr = centroids;
        while(cent_ptr != NULL){
            int curr_id = cent_ptr->id;
            memset(cent_ptr->val, 0, d * sizeof(double));
            for(i=0; i<n; i++){
                if (cluster_ids[i] != curr_id) continue;
                cluster_sizes[curr_id]+=1;
                add_vec(cent_ptr->val, X[i], d, 1.0);
            }
            cent_ptr = cent_ptr->next;
        };
        cent_ptr = centroids;
        while(cent_ptr != NULL){
            int curr_id = cent_ptr->id;
            scale_vec(cent_ptr->val, d, 1.0/cluster_sizes[curr_id]);
            cent_ptr = cent_ptr->next;
        };
    }

    num_clusters = centroids->id + 1;
...
double compute_regularization_term(PARAS myparas, double** X, double** dl_x, int num_samples, int dim, int* low_indices, int* upper_indices, PDATA* pd, int reg_type) {
    ...
    if (reg_type == 1 || reg_type == 3) {
        ...
        //ADITH
            double frobNorm = frob_norm(X, num_samples, dim);
            reg_objective += myparas.lambda * frobNorm * frobNorm;
        }
    ...
    if (myparas.regularization_type == 4) {
        ...
        sum_along_direct(X, centroid, num_samples, dim, 0);
        scale_vec(centroid, dim, 1.0/num_samples);
        ...
        if (dl_x != 0) {
            ...
            for (i = 0; i < myparas.num_points; i++) {
                for (j = low_indices[i]; j < upper_indices[i]; j++) {
                    for (k = 0; k < dim; k++) {
                        dl_x[i * num_samples + j][k] +=
                            -2.0 * myparas.lambda * (X[i * num_samples + j][k] - centroid[k]);
                    }
                }
            }
        }
            ...
            double frobNorm = frob_norm(X, num_samples, dim);
            double centroidNorm = vec_norm(centroid, dim);
            double innerProd = 0.0;
            for(i = 0; i < num_samples; i++)
                innerProd += innerprod(centroid, X[i], dim);
            reg_objective += myparas.lambda * (frobNorm * frobNorm + num_samples * centroidNorm * centroidNorm - 2.0*innerProd);
        } 
        ...
        //ADITH
        if (update_cluster_assignments == true){
            update_cluster_ids(X, num_samples, dim, myparas.nu_multiplier);
            update_cluster_assignments = false;
        }

        double** centroids = zerosarray(num_clusters,dim);
        for(i = 0; i < num_samples; i++) {
            add_vec(centroids[cluster_ids[i]], X[i], dim, 1.0/cluster_sizes[cluster_ids[i]]);
        }
        ...
        if (dl_x != 0) {
            ...
            for (i = 0; i < myparas.num_points; i++) {
                for (j = low_indices[i]; j < upper_indices[i]; j++) {
                    for (k = 0; k < dim; k++) {
                        dl_x[i * num_samples + j][k] +=
                            -2.0 * myparas.lambda * (X[i * num_samples + j][k] - centroids[cluster_ids[j]][k]);
                    }
                }
            }
        }
            ...
            double frobNorm = frob_norm(X, num_samples, dim);
            double clusterMeanNorms[num_clusters];
            for(i = 0; i < num_clusters; i++) {
                clusterMeanNorms[i] = vec_norm(centroids[i], dim);
            }
            for(i = 0; i < num_samples; i++) {
                reg_objective -= 2.0*myparas.lambda*innerprod(X[i], centroids[cluster_ids[i]], dim);
            }
            
            reg_objective += myparas.lambda * frobNorm * frobNorm;
            for(i = 0; i < num_clusters; i++)
                reg_objective += myparas.lambda*cluster_sizes[i]*clusterMeanNorms[i];
        } 

        Array2Dfree(centroids, num_clusters, dim);
    }
